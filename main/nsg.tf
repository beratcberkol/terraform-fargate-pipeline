...
resource "aws_security_group_rule" "nsg_lb_egress_rule_allowhttp" {
  description              = "Allow SG ${var.app}-${var.environment}-lb to connect to ${var.app}-${var.environment}-task on port ${var.container_port} and all connections"
  type                     = "egress"
  from_port                = "80"
  to_port                  = "80"
  protocol                 = "tcp"
